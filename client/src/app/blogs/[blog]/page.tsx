...
import { GET_BLOG_BY_ID } from "@/graphql/Blogs.query";
...
const BackgroundIcon = ({
  ...
  <div
    className={`fixed ${position} transform translate-x-12 -translate-y-12 text-indigo-200 z-0 opacity-70 pointer-events-none`}
  >
    {renderPageBackgroundIcon(IconName, size, size)}
  </div>
);
...
const BlogPage = ({ params }: BlogPageProps) => {
  ...
  const { loading, error, data } = useQuery(GET_BLOG_BY_ID, {
    variables: { documentId: resolvedParams.blog },
  });
  const sections = ["introduction", "main-content", "conclusion"];
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        <div className="flex gap-8">
          ...
          <div className="flex-1 max-w-6xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg">
            ...
            {/* Navigation Footer remains the same */}
            <footer className="mt-16 pt-8 border-t border-border">
              <div className="flex justify-between items-center">
